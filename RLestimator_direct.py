...
class RL_estimator(gym.Env):
    ...
    def reset(self, eval=False):
        ...
        self.initial_bias = np.random.uniform(-1, 1, size=(self.STATE_SIZE, 1)) * 10
        # bias and noise
        ...
        self.Cx_obs = np.diag([0.5, 0.5, 0.5, 0.5, 0.5, 0.5]) ** 2 * 8400
        ...
        self.Q_sim = np.diag([0.2, np.deg2rad(1.0)]) ** 2 * 0.01
        # self.Q_sim = np.diag([0.1, np.deg2rad(0.5)]) ** 2
        self.R_sim = np.diag([1.0, np.deg2rad(10.0)]) ** 2 * 0.01
        # self.R_sim = np.diag([0.1, np.deg2rad(1.0)]) ** 2
    ...
    def step(self, action):
        ...
        if cost < -8 or self.t >= self.total_time:
            done = True
        else:
            done = False
